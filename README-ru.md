...
Необязательные example `.dmi` файлы сохранены в `samples/dmi`.

- они лежат в репозитории для ручного открытия, отладки и быстрых проверок
- приложение не загружает их автоматически на старте
- startup flow по-прежнему начинается с пустого workspace

См. [samples/dmi/README.md](samples/dmi/README.md).
